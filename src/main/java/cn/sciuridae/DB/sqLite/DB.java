...
import static cn.sciuridae.Tools.stringTool.strToLong;
...
public class DB {
    ...
    public Map<String, List<String>> clearTree(List<Integer> Group) {
        String clearTree = "delete from tree where tree.userId in (select userQQ from teamMember where teamMember.id=";
    ...
    public synchronized int  creatGroup(Group group,String masterName){
        ...
        try {
            ...
            try {
                ...
                //把团长加进去，默认创的人是团长
                if (joinGroup(teamMember, group.getGroupid()) == 0) {
                    return 0;//已经有一个社团了
                }
                return 1;//创建成功
    ...
    public synchronized int joinGroup(teamMember teamMember, String groupQQ) {
        ...
        try {
            List<String> i1=h.executeQuery(sql,rowMapper2);
            if(i1.size()==0){
                try {
    ...
    public synchronized int outGroup(String userQQ) {
        String sql1 = "select rowid from teamMember where userQQ=\"" + userQQ + "\"";//找社团主键
        ...
        try {
            ...
            String sql5 = "delete from Token where teamMemberowId=" + rowid;
            h.executeUpdate(sql3,sql4,sql5);//删除资料
            return 0;
    ...
    public boolean isSuperPower( String QQ) {
        String sql = "select groupid from _group where groupMasterQQ=\"" + QQ + "\"";
        RowMapper<String> rowMapper = (rs, index) -> rs.getString("groupid");
     ...
     * @return
     */
    public synchronized void endFight(List<Integer> id){
        String sql="delete from progress where id=";
    ...
    public synchronized Map<String,Integer> searchKnife(String QQ,String time){
        String sql="select userQQ from teamMember where id=(select id from teamMember where userQQ=\""+QQ+"\");";
    ...
    public String groupStatue(String groupQQ) {
        ...
        RowMapper<Integer> rowMapper = (rs, index) -> rs.getInt("count(*)");
        String sql1 = "select count(*) from teamMember join _group on _group.id=teamMember.id where _group.groupid=\"";
        try {
            Group group = h.executeQuery(sql, groupId).get(0);
            int num = h.executeQuery(sql1+groupQQ+"\"", rowMapper).get(0);
            tips.append("工会名：").append(group.getGroupName()).append("\n工会会长qq：").append(group.getGroupMasterQQ()).append("\n工会创建时间：").append(group.getCreateDate()).append("\n工会成员数：").append(num);
        } catch (SQLException e) {
            e.printStackTrace();
        } catch (ClassNotFoundException e) {
            e.printStackTrace();
        } catch (IndexOutOfBoundsException e) {
            tips.append("还没有建立工会呢");
        }
     ...
     * @return
     */
    public int setAdmin(String Tragit) {
    ...
    public int downAdmin(String Tragit) {
        String sql = "update teamMember set power=0 where userQQ=\"" + Tragit + "\"";
        try {
            int i = h.executeUpdate(sql);
            return i;
        } catch (SQLException e) {
     ...
     * @return
     */
    public int searchIdByQQ(String QQ){
        String sql="select id from teamMember where userQQ =\""+QQ+"\"";
        RowMapper<Integer> rowMapper= (rs, index) -> rs.getInt("id");
     ...
     * @return
     */
    public String searchGroupQQByQQ(String QQ){
        String sql="select _group.groupid from teamMember join _group on _group.id=teamMember.id where teamMember.userQQ =\""+QQ+"\"";
        RowMapper<String> rowMapper= (rs, index) -> rs.getString("groupid");
     ...
     * @return
     */
    public int searchTeamMemberowIdByQQ(String QQ){
        String sql="select rowid from teamMember where userQQ =\""+QQ+"\"";
        RowMapper<Integer> rowMapper= (rs, index) -> rs.getInt("rowid");
    ...
    //和上面相反
    public String searchTeamMemberowIdByQQ(int TeamMemberowId){
        String sql="select userQQ from teamMember where  rowid=\""+TeamMemberowId+"\"";
        RowMapper<String> rowMapper= (rs, index) -> rs.getString("userQQ");
    ...
    /**
     * 检测密匙power 约定1为普通职员2为组长3为部长4为董事会
     * @param token
     * @return
     */
    public Power checkPower(String token){
        String sql="select teamMemberowId from Token where token=\""+token+"\"";
        RowMapper<Integer> row= (rs, index) -> rs.getInt("teamMemberowId");
        Power power=null;
        int teamId;
        try {
            teamId=h.executeQuery(sql,row).get(0);
            String qq=searchTeamMemberowIdByQQ(teamId);//得到这个加 密 字 符的qq号
            String sql1="select id from _group where groupMasterQQ=\""+qq+"\"";//工会长权限验证
            RowMapper<Integer> rowMapper= (rs, index) -> rs.getInt("id");
    ...
    public class Power{
        private int power;
        private int teamMemberowId;

        public Power(int power, int teamMemberowId) {
            this.power = power;
            this.teamMemberowId = teamMemberowId;
        }
        ...
        public int getteamMemberowId() {
            return teamMemberowId;
        }

        public void setteamMemberowId(int teamMemberowId) {
            this.teamMemberowId = teamMemberowId;
        }
    }
     ...
     * @return
     */
    public String searchQQById(int id){
        String sql="select userQQ from teamMember where rowid="+id;
        RowMapper<String> rowMapper= (rs, index) -> rs.getString("userQQ");
        String QQ=null;

        try {
            QQ=h.executeQuery(sql,rowMapper).get(0);
        } catch (SQLException e) {
            e.printStackTrace();
        } catch (ClassNotFoundException e) {
            e.printStackTrace();
    ...
    public List<teamMember> getTeamByRowid(int rowid){
        ...
        try {
            teamMember=h.executeQuery(sql,rowMapper);
        } catch (SQLException e) {
            e.printStackTrace();
        } catch (ClassNotFoundException e) {
            e.printStackTrace();
    ...
    /**
     * 获取这个人在会里的power 1为普通民工2为工头3为承包商
     * @param row
     * @return
     */
    public int getPower(int row){
        ...
        try {
            ...
            if(teamMember.isPower()){
                power=2;
                String sql1="select id from _group where groupMasterQQ=\""+teamMember.getUserQQ()+"\"";
                if(h.executeQuery(sql1,rowMapper1).size()>0){
                    power=3;
                }
            }
        } catch (SQLException e) {
